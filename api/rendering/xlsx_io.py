...
def build_sheet_workbook(title: str, content: str) -> bytes:
    ...
    **Formulas are written as formulas, not as their frozen values.** An .xlsx is a working
    ...
    """
    from api.rendering import render  # noqa: PLC0415 — avoids a circular import at module load
    ...
    for sheet in book["sheets"]:
        ...
        for r, row in enumerate(sheet["data"], start=1):
            for c, value in enumerate(row, start=1):
                ...
                if note:
                    cell.comment = Comment(note, "Auditrail")
...
def _number_format(fmt: str) -> str | None:
    ...
    from api.rendering import render  # noqa: PLC0415 — same circular-import dodge as the caller
